...
export async function POST(request: NextRequest, { params }: { params: Promise<{ businessId: string }> }) {
  try {
    ...
    while (hasMore) {
      const products = await wooApi.getProducts(page, 100)
      console.log(`Woo products page ${page}:`, products) // 🔎 log fetched products
    ...
    console.log("First transformed product:", transformedProducts[0]) // 🔎 log transformation result

    // Delete existing products (⚠️ dangerous: only for debugging!)
    await supabase.from("products").delete().eq("business_id", businessId)
    ...
    for (let i = 0; i < transformedProducts.length; i += batchSize) {
      ...
      if (insertError) {
        console.error("❌ Error inserting product batch:", insertError, batch[0])
        continue
      }

      console.log(`✅ Inserted batch of ${batch.length} products`)
      syncedCount += batch.length
    }
